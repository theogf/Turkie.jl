...
function TurkieParams(model; kwargs...) # Only needed for Turing models
    ...
    return TurkieParams(Dict(Pair.(keys(variables), Ref([:trace, :hist]))); kwargs...)
end
...
# Create a scene and a layout
    ...
    for (i, (variable, plots)) in enumerate(params.vars)
        ...
        for (j, p) in enumerate(plots)
            ...
            elseif p == :hist
                ...
                # end
                hist = lift(iter; init = KHist(nbins)) do i
                    fit!(hist[], last(data[variable]))
                    # N = length(data[variable])
                    # StatsBase.normalize(fit(Histogram, Float64.(data[variable]); nbins = nbins); mode = :pdf)
                end
                x = lift(hist; init = collect(range(0.0, 1.0, length = nbins))) do h
                    return keys(h.bins)
                end
                y = lift(hist; init = zeros(Float64, nbins)) do h
                    return values(h.bins)
                end
                
                barplot!(axes_dict[(variable, p)], x, y, color = std_colors[i])
    ...
    lift(iter) do i
        if i > 3 # To deal with autolimits a certain number of samples are needed
